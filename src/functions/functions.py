...
def yt_finder(snippet=None, duration="any", order="relevance"):
    ...
    req = requests.get(
        f"""https://www.googleapis.com/youtube/v3/search?part=snippet&q={snippet.replace(' ', '%20')}&key={sec.yt_api_key}&maxResults=5&type=video&videoDuration={duration}&order={order}"""
    )
    json_data = req.json()
